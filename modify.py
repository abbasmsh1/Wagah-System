...
@app.get('/check_its/')
def get_its(request: Request,its: int = Form(...), db: Session = Depends(get_db)):
...
def delete_booking(id: int = Form(...), db: Session = Depends(get_db)):
    booking = db.query(BookingInfo).filter(BookingInfo.id == id).first()
    if not booking:
